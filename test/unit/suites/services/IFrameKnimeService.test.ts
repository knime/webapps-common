...
describe("IFrameKnimeService", () => {
  ...
  describe("initialization", () => {
    ...
    it("registers extension config", () => {
      ...
      (knimeService as any).onMessageFromParent(
    ...
    it("waits for extentionConfig is resolved in waitForInitialization", async () => {
      ...
      await expect(knimeService.waitForInitialization()).resolves.toBe(
        undefined,
      );
    });
  });
  ...
  describe("postMessage communication", () => {
    ...
    beforeEach(() => {
      ...
      onCallServiceResponseSpy = jest.spyOn(
        knimeService,
        "onCallServiceResponse",
      );
    ...
    it("handles errors for responses without matching requests", () => {
      ...
      expect(sendErrorSpy).toHaveBeenCalledWith({
        ...
        code: "404",
        message:
          "Received callService response for non-existing pending request with id undefined",
