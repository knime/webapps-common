...
const buildIFrameKnimeServiceAdapter = () => {
  const callServiceSpy = jest
    .fn()
    .mockImplementation(() =>
      Promise.resolve({ result: JSON.stringify([1, 1, 2]) }),
    );
  ...
  jest
    .spyOn(iFrameKnimeServiceAdapter as any, "checkMessageSource")
...
describe("IFrameKnimeServiceAdapter", () => {
  describe("initialization", () => {
    it("creates IFrameKnimeServiceAdapter", () => {
      ...
      expect(iFrameKnimeServiceAdapter.extensionConfig).toEqual(
        extensionConfig,
      );
    ...
    it("posts init event on :ready type request", async () => {
      const { iFrameKnimeServiceAdapter, childSpy } =
        buildIFrameKnimeServiceAdapter();
    ...
    it("calls service when receiving :callService type events", async () => {
      const { iFrameKnimeServiceAdapter, childSpy, callServiceSpy } =
        buildIFrameKnimeServiceAdapter();
      const serviceParams = [
      ...
      window.postMessage(
        { type: `${UI_EXT_POST_MESSAGE_PREFIX}:callService`, payload },
        "*",
      );
    ...
    it("pushes events", async () => {
      const { iFrameKnimeServiceAdapter, pushEventSpy } =
        buildIFrameKnimeServiceAdapter();
