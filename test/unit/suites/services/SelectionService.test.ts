  ...
  describe("selection checkbox handling methods", () => {
    describe("onInit", () => {
      ...
      it("initializes onSelectionChangeCallback and adds it to the callbackMap when subscribeToSelection", () => {
        ...
        expect((selectionService as any).onSelectionChangeCallback).toEqual(
          onSelectionChangeCallback,
        );
        expect(
          (selectionService as any).callbackMap.has(onSelectionChangeCallback),
        ).toEqual(true);
        expect((selectionService as any).currentPublishSelection).toEqual(
          publishSelection,
        );
    ...
    describe("onSettingsChange", () => {
      ...
      beforeEach(() => {
        ...
        selectionService = new SelectionService(knime);
        onSelectionChangeCallback = jest.fn();
        getCurrentSelectionCallback = jest
          .fn()
          .mockReturnValue(["Row1", "Row4", "Row20"]);
      ...
      it("does nothing when onInit was not called before", () => {
        ...
        const addOnSelectionChangeCallbackSpy = jest.spyOn(
          selectionService,
          "addOnSelectionChangeCallback",
        );
        const removeOnSelectionChangeCallbackSpy = jest.spyOn(
          selectionService,
          "removeOnSelectionChangeCallback",
        );

        selectionService.onSettingsChange(
          getCurrentSelectionCallback,
          clearSelectionCallback,
          newPublishSelection,
          newSubscribeToSelection,
        );
      ...
      it("clears the current selection when subscribeToSelection changes to checked and adds the callback", () => {
        ...
        expect(addOnSelectionChangeCallbackSpy).toHaveBeenCalledWith(
          onSelectionChangeCallback,
        );
      ...
      it("does not clear the selection when subscribeToSelection changes to unchecked and removes the callback", () => {
        ...
        expect(removeOnSelectionChangeCallbackSpy).toHaveBeenCalledWith(
          onSelectionChangeCallback,
        );
        expect(replaceSpy).not.toHaveBeenCalled();
        expect(clearSelectionCallback).not.toHaveBeenCalled();
