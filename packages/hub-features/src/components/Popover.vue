...
const closeMenu = () => {
  if (expanded.value) {
    // vue-clickaway always triggers so we check again
...
<template>
  ...
  >
    ...
    <div v-if="expanded" v-on-click-outside="closeMenu" class="content">
      <slot name="content" />
      <FunctionButton
        v-if="hasCloseButton"
        class="closer"
...
.popover {
  ...
  &.expanded {
    z-index: 1; /* stacking context of ActionBar.vue .options which currently is the only usage of this component */
