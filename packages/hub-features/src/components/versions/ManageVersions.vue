<script setup lang="ts">
// TODO: HUB port last synced on 2025-02-04

import { FunctionButton } from "@knime/components";
import CloseIcon from "@knime/styles/img/icons/close.svg";
import HistoryIcon from "@knime/styles/img/icons/history.svg";

import { CURRENT_STATE_VERSION } from "../../common/constants";

import CurrentState from "./CurrentState.vue";
import VersionHistory from "./VersionHistory.vue";
import type {
  ItemSavepoint,
  NamedItemVersion,
  WithAvatar,
  WithLabels,
} from "./types";

type ManageVersionsProps = {
  hasUnversionedChanges: boolean;
  unversionedSavepoint: ItemSavepoint & WithAvatar & WithLabels;
  currentVersion: NamedItemVersion["version"] | null;
  versionHistory: Array<NamedItemVersion & WithAvatar & WithLabels>;
  loading: boolean;
  hasLoadedAllVersions: boolean;
  hasAdminRights: boolean;
  hasEditCapability: boolean;
};

defineProps<ManageVersionsProps>();

defineEmits<{
  (e: "close"): void;
  (e: "loadAll"): void;
  (e: "create"): void;
  (e: "delete", version: NamedItemVersion): void;
  (e: "restore", version: NamedItemVersion): void;
  (e: "select", version: NamedItemVersion["version"]): void;
}>();
</script>

<template>
  <div class="manage-versions-container">
    <FunctionButton class="close" @click="$emit('close')">
      <CloseIcon />
    </FunctionButton>

    <div class="manage-versions">
      <div class="header">
        <HistoryIcon class="history-icon" /><!--
        -->
        <h4>Version history</h4>
      </div>
      <div class="overflow-container">
        <div v-if="hasUnversionedChanges" class="changes">
          <CurrentState
            :has-edit-capability
            :is-selected="currentVersion === CURRENT_STATE_VERSION"
            :current-state-savepoint="unversionedSavepoint"
            @select="$emit('select', CURRENT_STATE_VERSION)"
            @create-version="$emit('create')"
          />
        </div>
        <div class="versions">
          <VersionHistory
            :selected-version="currentVersion"
            :version-history
            :loading
            :has-unversioned-changes
            :has-loaded-all-versions
            :has-admin-rights
            :has-edit-capability
            @delete="(v) => $emit('delete', v)"
            @restore="(v) => $emit('restore', v)"
            @load-all="$emit('loadAll')"
            @select="(v) => $emit('select', v ?? CURRENT_STATE_VERSION)"
          />
        </div>
      </div>
    </div>
  </div>
</template>

<style lang="postcss" scoped>
.manage-versions-container {
  position: relative;
  background-color: var(--knime-gray-light-semi);
  height: 100%;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  align-items: flex-start;
  overscroll-behavior: none;
  isolation: isolate;

  & .close {
    position: absolute;
    top: 6px;
    right: 6px;
    z-index: 3;
  }

  & .manage-versions {
    display: flex;
    flex-direction: column;
    width: 100%;

    & .header {
      background-color: var(--knime-white);
      z-index: 2;
      padding: 32px 30px 20px;

      & h4 {
        display: inline;
        margin: 0;
        font-size: 22px;
        line-height: 26px;
      }

      & .history-icon {
        margin-right: 9px;
        vertical-align: sub;
        width: 24px;
        height: 24px;
        stroke-width: calc(32px / 25);
        stroke: var(--knime-masala);
        position: relative;
        top: 1px;
      }
    }

    & .overflow-container {
      overflow-y: auto;
      flex: 0 1 auto;

      /* height: 100%; TODO: offset the expected height of the sidebar header on HUB */
    }

    & .changes {
      background-color: var(--knime-white);
      width: 100%;
      padding: 10px 30px 30px;
    }

    & .versions {
      width: 100%;
      padding: 30px;
    }
  }
}
</style>
