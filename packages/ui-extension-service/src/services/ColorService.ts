...
export class CIELabColorHandler extends NumericColorHandler<StopValuesCIELabModel> {
  public getColor(value: number | null): string {
    ...
    let i = 0;
    while (
      i < stopValues.length - 1 &&
      !(value >= stopValues[i] && value <= stopValues[i + 1])
    ) {
      i++;
    }

    const value0 = stopValues[i];
    const color0 = stopColorsCIELab[i];
    const value1 = stopValues[i + 1];
    const color1 = stopColorsCIELab[i + 1];
    if (value1 === value0) {
