...
type AvatarProps = {
   ...
   * External urls are recognized if they start with `http[...]` or if `NuxtLink` is available.
   */
  linkUrl?: string | null;
};
...
const hasExternalUrl = computed(() => {
  return typeof props.linkUrl === "string" && props.linkUrl.startsWith("http");
});
...
const rootComponent = computed(() => {
  ...
  return hasExternalUrl.value ? "a" : resolveNuxtLinkComponent();
});
</script>
...
<template>
  <Component
    :is="rootComponent"
    v-bind="hasExternalUrl ? { href: linkUrl } : { to: linkUrl }"
