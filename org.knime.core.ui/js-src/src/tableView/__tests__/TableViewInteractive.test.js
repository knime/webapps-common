...
describe("TableViewInteractive.vue", () => {
  ...
  const findTableUIWithAutoSizeCalculation = (wrapper) =>
    wrapper
  ...
  describe("initialization and data fetching", () => {
    ...
    it("renders the TableUIWithAutoSizeCalculation and passes the correct props", async () => {
      ...
      const { data, currentSelection, dataConfig, tableConfig } =
        tableUIWithAutoSizeCalculation.vm.$props;
  ...
  describe("settings change event", () => {
    ...
    it("updates displayed columns on displayed columns change", async () => {
      ...
      expect(
        findTableUIWithAutoSizeCalculation(wrapper).vm.tableConfig.pageConfig
          .columnCount,
    ...
    it("adds additional column indicating that columns were skipped", async () => {
      ...
      expect(
        tableUIWithAutoSizeCalculation.vm.tableConfig.pageConfig.columnCount,
      ).toBe(dataRequestResult.columnCount);
    ...
    it("updates columnDataTypeIds on displayes columns update", async () => {
      ...
      expect(
        findTableUIWithAutoSizeCalculation(wrapper).vm.dataConfig,
      ).toMatchObject({
        columnConfigs: newColumnConfig,
      });
    });
  ...
  describe("sorting and pagination", () => {
    ...
    it("disables sorting", async () => {
      ...
      const { tableConfig } =
        findTableUIWithAutoSizeCalculation(wrapper).vm.$props;
      expect(tableConfig).toMatchObject({
        subMenuItems: [],
        pageConfig: {
  ...
  describe("global and column search", () => {
    ...
    beforeEach(async () => {
      ...
      setColumnFilter = (colIndex, filterVal) => {
        tableUIWithAutoSizeCalculation.vm.$emit(
          "columnFilter",
          colIndex,
          filterVal,
        );
    ...
    it("requests new data on global search", () => {
      ...
      findTableUIWithAutoSizeCalculation(wrapper).vm.$emit(
        "search",
        globalSearchTerm,
      );
  ...
  describe("column resizing", () => {
    ...
    const setColumnWidthSettings = async (wrapper, { availableWidth }) => {
      await wrapper.setData({
    ...
    beforeEach(async () => {
      wrapper = await shallowMountInteractive(context);
      tableUIWithAutoSizeCalculation = wrapper.findComponent(
        TableUIWithAutoSizeCalculation,
      );
      setColumnWidth = async (colIndex, newSize) => {
        await tableUIWithAutoSizeCalculation.vm.$emit(
          "column-resize",
          colIndex,
          newSize,
        );
    ...
    it.each([
      ...
      async (settings, columnWidth) => {
        ...
        expect(
          wrapper.findComponent(TableViewDisplay).vm.columnSizes,
        ).toStrictEqual([
          MIN_COLUMN_SIZE,
          MIN_COLUMN_SIZE,
          columnWidth,
    ...
    it("updates column sizes from availableWidth", async () => {
      ...
      await wrapper
        .findComponent(TableUIWithAutoSizeCalculation)
        .vm.$emit("all-columns-resize", allColumnsResizeSize);
      expect(
        wrapper.findComponent(TableViewDisplay).vm.columnSizes,
      ).toStrictEqual([
        0,
    ...
    it("sets columnResizeActive state", () => {
      const columnResizeActive =
        wrapper.findComponent(TableViewDisplay).vm.columnResizeActive;
  ...
  describe("calculation of fit content column sizes", () => {
    ...
    it("correctly converts the image dimensions of the first row", async () => {
      initialDataMock.table.firstRowImageDimensions = {
        col2: { widthInPx: 40, heightInPx: 80 },
        col4: { widthInPx: 180, heightInPx: 140 },
      };
      const wrapper = await shallowMountInteractive(context);
      expect(wrapper.vm.fixedColumnSizes).toStrictEqual({
        col2: 40,
        col4: 102,
      });
    });
