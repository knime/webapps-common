...
const getDataColumnSizes = ({
    ...
    const defaultColumnSize = Math.max(DEFAULT_COLUMN_SIZE, availableSpace / displayedColumns.length);
    const currentDefaultColumnSize = defaultColumnSizeOverride || defaultColumnSize;
    return displayedColumns.reduce((columnSizes, columnName) => {
        columnSizes.push(columnSizeOverrides[columnName] || currentDefaultColumnSize);
        return columnSizes;
    }, [] as number[]);
};
...
export default ({
    ...
    const indexColumnSize = columnSizeOverrides[INDEX.id] || initialIndexColumnSize;
    const rowKeyColumnSize = columnSizeOverrides[ROW_ID.id] || initialRowKeyColumnSize;
    const remainingSkippedColumnSize = columnSizeOverrides[SKIPPED_REMAINING_COLUMNS_COLUMN.id] ||
        initialRemainingSkippedColumnSize;

    const currentColumnSizes = [indexColumnSize, rowKeyColumnSize]
        .concat(getDataColumnSizes({
            availableSpace: initialTableColumnsSizeTotal,
            displayedColumns,
            columnSizeOverrides,
            defaultColumnSizeOverride
        }))
        .concat(indicateRemainingColumnsSkipped ? [remainingSkippedColumnSize] : []);

    const indexColumnIsOnlyColumn = currentColumnSizes.length === 2 && !settings.showRowKeys;
