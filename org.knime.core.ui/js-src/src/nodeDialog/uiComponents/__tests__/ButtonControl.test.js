...
describe("ButtonControl", () => {
  ...
  const mountButtonControl = ({ props, getDataMock }) =>
    mountJsonFormsComponent(ButtonControl, { props, provide: { getDataMock } });
  ...
  beforeEach(async () => {
    ...
    component = await mountButtonControl({ props, getDataMock: getData });
  ...
  describe("dependencies to other settings", () => {
    ...
    beforeEach(() => {
      ...
      getData = vi.fn(() => getDataResult);
      const comp = mountButtonControl({ props, getDataMock: getData });
