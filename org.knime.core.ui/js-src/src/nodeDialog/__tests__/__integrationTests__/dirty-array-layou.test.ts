...
import { controllingFlowVariableState, exposedFlowVariableState } from "@@/test-setup/utils/integration/dirtySettingState";
...
describe("dirty array layout", ()=> {
      ...
      const clickAddButton = (wrapper: Wrapper) => {
        const plusButton: HTMLButtonElement = wrapper.find(".array").findComponent(PlusIcon).element.parentElement! as HTMLButtonElement;
        return plusButton.click();
      };
    

      const clickLastTrashButton = (wrapper: Wrapper) => {
        const lastTrashButton: HTMLButtonElement = wrapper.find(".array").findAllComponents(TrashIcon).at(-1).element.parentElement! as HTMLButtonElement;
      ...
      beforeEach(async () => {
        dirtyStates = []
         vi.spyOn(DialogService.prototype, "registerSettings").mockImplementation(() => registerSetting)
         mockInitialData();
      ...
      const isClean = () => dirtyStates.map(({getValue, initialValue}) => getValue() === initialValue).filter(clean => clean === false).length === 0
