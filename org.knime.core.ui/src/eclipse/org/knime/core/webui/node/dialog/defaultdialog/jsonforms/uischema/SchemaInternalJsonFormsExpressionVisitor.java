 ...
 */
final class SchemaInternalJsonFormsExpressionVisitor implements JsonFormsExpressionVisitor {

    private final DefaultNodeSettingsContext m_context;

    SchemaInternalJsonFormsExpressionVisitor(final DefaultNodeSettingsContext context) {
        m_context = context;
    }

    @Override
    public ObjectNode visit(final ScopedExpression scopedExpression) {
        final var path = scopedExpression.node().getPath();
        ...
        currentNode.setAll(createCondition(scopedExpression, m_context));
        return objectNode;
    }

    @Override
    public ObjectNode visit(final ConstantExpression constantExpression) {
        return createCondition(constantExpression);
    }
