...
/**
 * A class that provides an array of possible values of a {@link ChoicesWidget} based on the current
...
public non-sealed interface StringChoicesStateProvider extends ChoicesStateProvider<IdAndText[]> {
    ...
    @Override
    default IdAndText[] computeState(final DefaultNodeSettingsContext context) {
        return Arrays.stream(choices(context)).map(IdAndText::fromId).toArray(IdAndText[]::new);
    }
