...
describe('Twinlist.vue', () => {
    ...
    describe('controls', () => {
        ...
        it('makes the invalid columns disappear upon moving them to the left', async () => {
            ...
            expect(left.props('possibleValues').map(x => x.id)).not.toContain('invalidId');
            expect(right.props('possibleValues').map(x => x.id)).not.toContain('invalidId');
        });
    ...
    describe('unknown values', () => {
        ...
        const expectUnknownValuesAreIncluded = (wrapper) => {
            expect(
                wrapper.findAllComponents(MultiselectListBox).at(0).find('[role="bottom-box"]').exists()
            ).toBeFalsy();
            expect(
                wrapper.findAllComponents(MultiselectListBox).at(1).find('[role="bottom-box"]').exists()
            ).toBeTruthy();
        };
        ...
        describe('excluded', () => {
            ...
            it('moves unkown values right if selected on moveRightButton', async () => {
                const wrapper = mount(Twinlist, { props });
                const leftBox = wrapper.findAllComponents(MultiselectListBox).at(0);
                await leftBox.find('[role="bottom-box"]').find('[role="option"]').trigger('click');
        ...
        describe('included', () => {
            ...
            it('moves unknown values left on moveAllLeftButton', async () => {
                ...
                await rightBox.find('[role="bottom-box"]').find('[role="option"]').trigger('click');
                await wrapper.find({ ref: 'moveAllLeft' }).trigger('click');
                expectUnknownValuesAreExcluded(wrapper);
