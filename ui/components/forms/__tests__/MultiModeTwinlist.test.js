...
describe('MultiModeMultiModeTwinlist.vue', () => {
    ...
    const expectTwinlistIncludes = (wrapper, expectedLeft, expectedRight) => {
        let boxes = wrapper.findComponent(Twinlist).findAllComponents(MultiselectListBox);
    ...
    it('renders', () => {
        ...
        expect(wrapper.findComponent(Twinlist).findAllComponents(MultiselectListBox).length).toBe(2);
        expect(
            wrapper.findComponent(Twinlist).findAllComponents(MultiselectListBox).at(0).vm.$props.possibleValues.length
        ).toBe(2);
        expect(
            wrapper.findComponent(Twinlist).findAllComponents(MultiselectListBox).at(1).vm.$props.possibleValues
        ).toStrictEqual(
    ...
    describe('mode selection', () => {
        ...
        it('hides type selection mode if wanted', () => {
            ...
            expect(wrapper.findComponent(ValueSwitch).exists()).toBeTruthy();
            expect(wrapper.findAll('div.label label').map(l => l.text())).not.toContain('Type');
        });
    ...
    describe('type selection', () => {
        ...
        it('selects via type matching', async () => {
            ...
            wrapper.findComponent(Checkboxes).vm.$emit('update:modelValue', ['StringValue']);
            await wrapper.vm.$nextTick();
    ...
    describe('unknown columns', () => {
        ...
        it('renders and excludes unkown columns by default', () => {
            ...
            const wrapper = mount(MultiModeTwinlist, { propsData });
            expect(wrapper.findComponent(Twinlist).vm.initialIncludeUnknownValues).toBeFalsy();
        });
        ...
        it('emits includeUnknownValuesInput event', async () => {
            const wrapper = mount(MultiModeTwinlist, { propsData });
            await wrapper.findComponent(Twinlist).vm.$emit('includeUnknownValuesInput', true);
