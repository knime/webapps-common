...
describe('MenuItemsBase.vue', () => {
    ...
    describe('clicking menu items', () => {
        ...
        it('emits item-click', async () => {
            ...
            expect(wrapper.emitted('item-click')[0]).toStrictEqual([expect.anything(), items[0], id]);

            await wrapper.findAll('li')[1].trigger('click');
            expect(wrapper.emitted('item-click')[1]).toStrictEqual([expect.anything(), items[1], id]);

            await wrapper.findAll('li')[2].trigger('click');
            expect(wrapper.emitted('item-click')[2]).toStrictEqual([expect.anything(), items[2], id]);
        });
    ...
    it('marks current focused item', () => {
        ...
        const currentfocusedElement = wrapper.findAll('li')[focusedItemIndex].find('*');
        expect(currentfocusedElement.classes().includes('focused')).toBeTruthy();
        const nonfocusedElement = wrapper.findAll('li')[focusedItemIndex + 1].find('*');
